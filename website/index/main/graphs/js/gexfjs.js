...
function displayNode(_nodeIndex, _recentre) {
	...
	_str += '<ul style="margin: 10px 5px;">';
        /*for (var i in _d.attributes) {
            _str += '<li><b>' + strLang(i) + '</b> : ' + replaceURLWithHyperlinks( _d.attributes[i] ) + '</li>';
        }*/
	_str += '<li><b>Frequency in the Community Core</b>: ' + replaceURLWithHyperlinks( _d.attributes['d5'] ) + '</li>';
	_str += '<li><b>Final Estimated H-Index</b>: ' + replaceURLWithHyperlinks( _d.attributes['d2'] ) + '</li>';
	_str += '<li><b>Total of Publications</b>: ' + replaceURLWithHyperlinks( _d.attributes['d1'] ) + '</li>';
	_str += '<li><b>Component ID</b>: ' + replaceURLWithHyperlinks( _d.attributes['componentnumber'] ) + '</li>';
...
function loadGraph() {
    var querystring = document.location.hash.substr(document.location.hash.indexOf("?") + 1);
    var params = querystring.split('&');
    var sig = params[0].split('=')[1]
    var conf = params[1].split('=')[1]
    $("#sig").text(sig);
    $("#conf").text(conf);    
    $.ajax({
