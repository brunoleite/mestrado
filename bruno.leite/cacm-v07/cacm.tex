...
In other words, one could expect a large LCC in a research community in which authors often interact and collaborate, meaning that there exists at least one path among a large fraction of them.
...
More formally, the \textit{CoScore} of a researcher $r$ in a community $c$ during a period of time $t$, $CoScore_{r,c,t}$, is given by her h-index $h_r$ multiplied by the number of publications $r$ has in $c$ during $t$ ($\textrm{\#}publications_{r,c,t}$), as expressed by the following equation:
\begin{equation}
  \label{eq:core_score}
...
We have computed a ranking of the researchers that appear most often in the top 10\% of the \textit{CoScore} ranking over the years
